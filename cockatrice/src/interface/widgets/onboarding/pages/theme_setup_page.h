...
/**
 * First-run theme step. Reuses the same building blocks as Appearance
 ...
 *
 * Two behaviours specific to this page (deliberately not pushed down into
 * ThemeManager, to avoid changing app-wide behaviour for existing installs):
 *  - The scheme combo stays in sync when the selected theme itself changes,
 *    mirroring AppearanceSettingsPage's own listener for the same thing.
